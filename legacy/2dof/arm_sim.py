...
# State
x = np.array([q1, q2, dq1, dq2], dtype=float)
...
def rot_2d(theta, vect):
    ...
    rot_matrix = np.array([[ np.cos(theta), -np.sin(theta)],
                           [ np.sin(theta),  np.cos(theta)]])
    rot_vect = rot_matrix.dot(vect)
    return rot_vect


def transform_points(theta, pos, scale, pts):
    trans_pts = [scale * rot_2d(theta, pt) + pos for pt in pts]
    return trans_pts
...
def update_display():
    global x, tau1, tau2
    state_str = f"q1: {np.rad2deg(x[0]):.2f}, q2: {np.rad2deg(x[1]):.2f}, tau1: {tau1:.3f}, tau2: {tau2:.3f}"
    # Reset screen
    pygame.draw.rect(screen, (0,0,0) , screen.get_rect(), 0)
    # Define triangle vertices for robot arm in default position (theta = 0)
    arm_vertices = [(0, -1), (10, 0), (0, 1)]
    first_link = transform_points(x[0], (0, 0), 10, arm_vertices)
    DrawPolygon(screen, World2Screen(first_link), (0, 255, 255))
    # Use previous definition to draw second arm link
    second_link = transform_points(x[0] + x[1], first_link[1], 5, arm_vertices)
    DrawPolygon(screen, World2Screen(second_link), (255, 255, 0))
