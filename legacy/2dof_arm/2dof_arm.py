...
# --- Physics Variables --- 
# Controls
mv_user = array([0.0,0.0]) # This is the user set value for mv  (valve aperture in percent)
mv_out  = array([0.0,0.0]) # This is the physical value of the mv (valve aperture in area,  flow rate, or physical signal, e.g. driver current/voltage)
...
def update_display():
    ...
    state_str = 'q1: '+str(x[0]*180.0/pi)+', q2:'+str(x[1]*180.0/pi)+\
...
def xd_fun(x, t):
    ...
    return multiply(time_scaling,[qd1, qd2, qdd[0], qdd[1]])
   
def update_state():
    global x # State variables
    global ti, Ts
   
    x0 = array(x)  # Current state is the initial state
